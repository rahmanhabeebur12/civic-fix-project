...
const TIMELINE_STEPS = [
  ...
  { status: "AI_VERIFIED", labelKey: "timelineAiVerified" as const },
...
export default function TrackReportPage() {
  ...
  function load() {
    ...
    setError(null);
    api.trackReport(complaintId).then(setIssue).catch((e) => setError(e instanceof ApiError ? e.message : t.errorGeneric));
  }
  ...
  const reachedIndex = TIMELINE_STEPS.findIndex((s) => s.status === issue.status);
  const currentStepIndex = reachedIndex === -1 ? (issue.status === "REOPENED" ? 4 : TIMELINE_STEPS.length - 1) : reachedIndex;
